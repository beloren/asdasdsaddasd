...
for i in range(0, len(ores), 3):
    chunk = ores[i:i + 3]
    rows.append(["%s  (%s)" % (c("Crystal_" + k), rar[k]) for k in chunk] + [""] * (3 - len(chunk)))
...
TABLE(["Имя ассета", "Кто это", "Где стоит"], [
    ...
    [cfg["Merchant"].get("AssetName", "BankMerchant"), "Торговец у банка (Config.Merchant.AssetName)", "у банка"],
