...
""")
P("Контракт — как у `Crystal_Tier<N>` (раздел 2.5). Все имена руды (%d):" % len(ores))
rows = []
for i in range(0, len(ores), 3):
    chunk = ores[i:i + 3]
    rows.append(["`Crystal_%s`  (%s)" % (k, rar[k]) for k in chunk] + [""] * (3 - len(chunk)))
TABLE(["Руда", "Руда", "Руда"], rows, [1, 1, 1], code_cols=())
...
P("По 3 скина на тип (случайный при спавне): " + ", ".join("`Goblin_%s_1/_2/_3`" % t for t in gt) + ". Можно один номер; старое имя без номера тоже работает.")
TREE("""
...
└─ Motor6D: RootJoint, Neck, Left/Right Shoulder, Left/Right Hip  (желательно; нет — достроит)
""")
P("`Goblin_Golden_*` — элитный гоблин из золотого валуна; нет модели — берётся King и красится в оранжевый. Иконка всех гоблинов: `Config.Goblins.IconImage`.")
...
] + [[v["Asset"], "Декор: " + k] for k, v in cfg["Placeables"]["Decor"].items()]
...
TABLE(["Что", "Где", "Картинка / настройка"], [
    ["Кнопка магазина 🛒", "Топбар, первой (client/ShopDockButton)", "`UiTheme.Icons.Shop` (пусто — эмодзи)"],
